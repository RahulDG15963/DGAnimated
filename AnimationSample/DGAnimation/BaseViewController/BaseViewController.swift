...
class BaseViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        dG_closeButton = UIButton.init(frame: CGRect.init(x: 8, y: 20, width: 40, height: 40))
        ...
        dG_closeButton.setBackgroundImage(#imageLiteral(resourceName: "dg_pattern"), for: .normal)
        ...
        let dg_imageView = UIImageView.init(frame: dg_containerView.bounds)
        dg_imageView.image = #imageLiteral(resourceName: "dg_pattern")
        ...
        dG_indexLabel.font = UIFont.systemFont(ofSize: 15, weight: UIFontWeightMedium)
    ...
    func closeButtonClicked(button: UIButton) {
        self.dismiss(animated: true, completion: nil)
    }
    
    func nextButtonClicked(button: UIButton) {
    ...
    func previousButtonClicked(button: UIButton) {
        if dG_index > 0 {
            dG_collectionView.scrollToItem(at: IndexPath.init(row: dG_index - 1, section: 0), at: .centeredHorizontally, animated: true)
            dG_index = dG_index - 1
            self.dG_indexLabel.setUp(index: dG_index + 1, totalItems: self.dG_imageArray.count)
